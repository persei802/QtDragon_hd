...
def gcode_words():
    words = {'G0': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G1': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G2': ['X', 'Y', 'Z', 'I', 'J', 'K', 'R', 'P'],
             'G3': ['X', 'Y', 'Z', 'I', 'J', 'K', 'R', 'P'],
             'G4': ['P'],
             'G5': ['I', 'J', 'P', 'Q'],
             ...
             'G33': ['X', 'Y', 'Z', 'K', '$'],
             'G33.1': ['X', 'Y', 'Z', 'K', '$'],
             'G38.2': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G38.3': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G38.4': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G38.5': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G41': ['D'],
             'G41.1': ['D', 'L'],
             'G42': ['D'],
             'G42.1': ['D', 'L'],
             'G43': ['H'],
             'G43.1': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G43.2': ['H'],
             'G52': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G53': ['X', 'Y', 'Z', 'A', 'B', 'C', 'U', 'V', 'W'],
             'G64': ['P', 'Q'],
             'G73': ['X', 'Y', 'Z', 'R', 'Q', 'L'],
             'G74': ['X', 'Y', 'Z', 'R', 'L', 'P', '$'],
...
def mcode_descriptions(mcode):
    mcodeTitle = {'M1': M1,
                  ...
                  'M67': M67,
                  'M68': M68}
    if mcode in mcodeTitle:
        return mcodeTitle[mcode]
    else:
        return ''
# Maximum Width is 58
...
G17 = 'G17 Plane Select XY'

G18 = 'G18 Plane Select XZ'

G19 = 'G19 Plane Select YZ'

G17_1 = """G17.1 Plane Select UV
The UV plane does not support arcs.
"""

G18_1 = """G18.1 Plane Select WU
The WU plane does not support arcs.
"""

G19_1 = """G19.1 Plane Select VW
...
Turn cutter compensation off. If tool compensation was on,
the next move must be a linear move and longer than the tool diameter.
"""
...
G54 = 'G54 Select Coordinate System 1'

G55 = 'G55 Select Coordinate System 2'

G56 = 'G56 Select Coordinate System 3'

G57 = 'G57 Select Coordinate System 4'

G58 = 'G58 Select Coordinate System 5'

G59 = 'G59 Select Coordinate System 6'

G59_1 = 'G59.1 Select Coordinate System 7'

G59_2 = 'G59.2 Select Coordinate System 8'

G59_3 = 'G59.3 Select Coordinate System 9'
...
M1 = """M1
Pause a running program temporarily if the optional stop
switch is on.
"""
...
M3 = """M3 S $
Start the selected spindle clockwise at the S speed.
If the $ is omitted, the behaviour is exactly as normal for
a single spindle machine.
"""

M4 = """M4 S $
Start the selected spindle counterclockwise at the S speed.
If the $ is omitted, the behaviour is exactly as normal for
a single spindle machine.
"""

M5 = """M5 $ Stop the selected spindle.
If the $ is omitted, the behaviour is exactly as normal for
a single spindle machine.
"""
...
M6 will stop the spindle and prompt the user to change
the tool based on the last T- number programmed.
"""

M7 = """M7 Turn mist coolant on.
M7 controls iocontrol.0.coolant-mist pin.
"""

M8 = """M8 Turn flood coolant on.
M8 controls iocontrol.0.coolant-flood pin.
 """

M9 = 'M9 Turn both M7 and M8 off.'

M19 = """M19 Orient Spindle
R Position to rotate to from 0, valid range is 0-360 degrees
Q Number of seconds to wait until orient completes.
    If spindle.N.is-oriented does not become true within Q timeout an error occurs.
P Direction to rotate to position.
$ The spindle to orient.
    (only determines which HAL pins carry the spindle position commands)
"""

M48 = """M48 Speed and Feed Override Control
Enable the spindle speed and feed rate override controls.
"""

M49 = """M49 Speed and Feed Override Control
Disable both controls.
"""

M50 = """M50 Pn Feed Override Control
n=0 - disable the feed rate override control.
n=1 - enable the feed rate override control.
      The P1 is optional.
"""

M51 = """M51 Pn $ Spindle Speed Override Control
n=0 - disable the spindle speed override control program.
n=1 - enable the spindle speed override control for the selected spindle.
      The P1 is optional.
"""

M52 = """M52 Pn Adaptive Feed Control
n=0 - stop using adaptive feed.
n=1 - use an adaptive feed. The P1 is optional.
"""

M53 = """M53 Pn Feed Stop Control
n=0 - disable the feed stop switch. The state of motion.feed-hold will have
      no effect on feed when M53 is not active.
n=1 -  enable the feed stop switch. The P1 is optional. Enabling the feed stop
       switch will allow motion to be interrupted by means of the feed stop control.
       In LinuxCNC, the HAL pin motion.feed-hold is used for this purpose.
       A true value will cause the motion to stop when M53 is active.
"""

M61 = """M61 Q Set Current Tool
Change the current tool number while in MDI or Manual mode without a tool change.
"""

M62 = """M62 P Digital Output Control
Turn on digital output synchronized with motion.
"""

M63 = """M63 P Digital Output Control
Turn off digital output synchronized with motion.
"""

M64 = """M64 P Digital Output Control
Turn on digital output immediately.
"""

M65 = """M65 P Digital Output Control
Turn off digital output immediately.
"""

M66 = """M66 Wait On Input
...
M67 = """M67 Analog Output Synchronized
